...
if (global.DEV) {
  try { require.resolve('eslint') } catch (e) {
    ...
    try {
      ...
      console.error('! Установите dev пакеты вручную выполнив в терминале: npm i\n')
    }
  }
}

const debug = require('debug')('vcoinx:main')
const url = require('url'),
  AutoUpdater = require('auto-updater'), // TODO: Вынести автообновлятор в отдельный модуль
  open = require('open'),
  {
    VK,
  } = require('vk-io')
...
autoupdater.on('update.extracted', function () {
  ...
  if (depDiff.count > 0) con('У обновленной версии были изменены зависимости.', 'white', 'Red')
})
...
if (checkUpdates) autoupdater.fire('check')

function notifyToRestart () {
  if (needRestart) con('Для применения обновления требуется перезапуск бота!', 'white', 'Green')
}
...
vCoinWS.onMissClickEvent(() => {
  ...
  if (++missCount > 20) forceRestart(4e3)
  if (++missCount > 10) {
    autobeep && beep()
...
vCoinWS.onReceiveDataEvent(async (place, score) => {
  ...
  if (place > 0 && !rl.isQst) {
    ...
    if (autoBuy && vCoinWS.tick <= limitCPS && score > 0) {
      for (var i = 0; i < autoBuyItems.length; i++) {
        if (miner.hasMoney(autoBuyItems[i])) {
          try {
            let result = await vCoinWS.buyItemById(autoBuyItems[i])
    ...
    if (advertDisp === 0x1) process.exit()
...
function forceRestart (t, force) {
  ...
  if (xRestart || force) startBooster(t)
}
...
rl.on('line', async (line) => {
  ...
  switch (line.trim().toLowerCase()) {
    ...
    case 'start':
    case 'run':
      if (vCoinWS.connected) return con('VCoinX уже запущен и работает!')
      xRestart = true
      startBooster()
      break
    case 'b':
    case 'buy':
      ...
      for (var i = 0, j = array.length; i < j; i++) {
        ...
        try {
          ...
          miner.updateStack(result.items)
          if (result && result.items) delete result.items
          con('Новая скорость: ' + formatScore(result.tick, true) + ' коинов / тик.')
        } catch (e) {
          if (e.message === 'NOT_ENOUGH_COINS') con('Недостаточно средств для приобретения.', true)
          else if (e.message === 'ITEM NOT FOUND') con('Предмет не найден.', true)
    ...
    case 'autobuyitem':
      ...
      let autobuyarray = item.split(' ')
      for (let i = 0; i < autobuyarray.length; i++) {
...
// TODO: Переписать обработку аргументов CLI
...
async function smartBuyFunction (score) {
  if (tempDataUpdate.tmpPr == null) tempDataUpdate.tmpPr = 100 / tempDataUpdate.percentForBuy
...
function updateLink () {
  if (!IFRAME_URL || tforce) {
    ...
    (async function inVKProc (token) {
      ...
      try {
        if (!GROUP_ID) {
          var iframe_url = (await vk.api.apps.get({
            app_id: 6915965,
          ...
          if (response.original_url === 'https://vk.com/coin') {
            throw new Error('Указан некорректный ID группы или группа не подключила майнинг VKCoin!')
          }
        }
        if (!iframe_url) throw new Error('Не удалось получить ссылку на приложение.\n\t\tВозможное решение: Используйте расширенный токен.')
        let id = (await vk.api.users.get())[0]['id']
        if (!id) throw new Error('Не удалось получить ID пользователя.')
        USER_ID = id
        formatWSS(iframe_url)
        startBooster()
      } catch (error) {
        // TODO: Переделать на switch-case и укоротить
